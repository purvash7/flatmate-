    ...
    <>
      <header className="sticky top-0 z-40 bg-[#FAF8F4]/90 backdrop-blur-md border-b border-[#E6E3DE]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div id="brand-logo" onClick={() => setActiveTab('discover')} className="flex items-center gap-3 cursor-pointer group">
          ...
          {user ? (
            <nav className="hidden md:flex items-center gap-1 lg:gap-2">
              <button id="nav-btn-discover" onClick={() => setActiveTab('discover')} className={`px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all min-h-[44px] ${activeTab === 'discover' ? 'bg-[#E07A5F] text-white shadow-sm' : 'text-[#7A7D87] hover:text-[#2B2D42] hover:bg-white/80'}`}>
              ...
              <button id="nav-btn-profile" onClick={() => setActiveTab('profile')} className={`px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all min-h-[44px] ${activeTab === 'profile' ? 'bg-[#E07A5F] text-white shadow-sm' : 'text-[#7A7D87] hover:text-[#2B2D42] hover:bg-white/80'}`}>
                <User className="w-4 h-4" /><span>{profile?.name ? profile.name.split(' ')[0] : 'Profile'}</span>
              </button>
              <button id="nav-btn-settings" onClick={() => setActiveTab('settings')} className={`p-2 rounded-xl text-sm transition-all min-h-[44px] min-w-[44px] flex items-center justify-center ${activeTab === 'settings' ? 'bg-[#E07A5F] text-white' : 'text-[#7A7D87] hover:text-[#2B2D42] hover:bg-white/80'}`} title="Settings">
                <Settings className="w-5 h-5" />
              </button>
            </nav>
          ) : (
            <div className="flex items-center gap-3">
              <button id="header-login-btn" onClick={onOpenAuth} className="px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-[#E07A5F] hover:bg-[#D4694E] shadow-sm transition-all min-h-[44px]">Get Started</button>
            </div>
          )}
        </div>
      </header>
      ...
      {user && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t border-[#E6E3DE] px-3 py-2 pb-safe">
          <div className="flex items-center justify-around max-w-md mx-auto">
            <button id="mobile-nav-discover" onClick={() => setActiveTab('discover')} className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all min-w-[56px] min-h-[48px] ${activeTab === 'discover' ? 'text-[#E07A5F] font-bold' : 'text-[#7A7D87]'}`}>
            ...
            <button id="mobile-nav-profile" onClick={() => setActiveTab('profile')} className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all min-w-[56px] min-h-[48px] ${activeTab === 'profile' ? 'text-[#E07A5F] font-bold' : 'text-[#7A7D87]'}`}>
              <User className={`w-5 h-5 ${activeTab === 'profile' ? 'stroke-[2.5]' : ''}`} /><span className="text-[11px] mt-0.5">Profile</span>
            </button>
            <button id="mobile-nav-settings" onClick={() => setActiveTab('settings')} className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all min-w-[56px] min-h-[48px] ${activeTab === 'settings' ? 'text-[#E07A5F] font-bold' : 'text-[#7A7D87]'}`}>
              <Settings className={`w-5 h-5 ${activeTab === 'settings' ? 'stroke-[2.5]' : ''}`} /><span className="text-[11px] mt-0.5">Settings</span>
            </button>
          </div>
        </div>
      )}
